...
export default function ReviseTaskList() {
  // Extract taskStatuses and dispatch from the context using useTasks hook
  const { taskStatuses, dispatch } = useTasks();

  // Local state to handle the sort order (ascending or descending)
  const [sortOrder, setSortOrder] = useState("asc");

  // Find tasks categorized under "Revised" from taskStatuses
  const reviseTasks = taskStatuses.filter(
    (task) => task.category === "Revised"
  );

  // Sort the tasks based on the dueDate and the current sortOrder state
  const sortedReviseTasks = [...reviseTasks].sort((a, b) => {
  ...
  const toggleSortOrder = () => {
    ...
    dispatch({ type: "SORT_TASKS", category: "Revised", order: newOrder }); // Dispatch to the context
    setSortOrder(newOrder); // Update the local state with the new order
  };

  return (
    <div className="mb-4 w-full px-2 sm:w-1/2 md:w-1/4">
      {/* Wrapper for the "Revised" task list */}
      <div className="rounded-lg bg-rose-500 p-4">
        {/* Header containing the "Revised" title and the number of tasks */}
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-lg font-semibold">
            Revise ({sortedReviseTasks.length}) {/* Display number of tasks */}
          </h3>
        ...
        <div>
          ...
          {sortedReviseTasks.length === 0 ? (
            <NoTaskFound textColor="text-[#f43f5e]" />
          ) : (
            // Otherwise, map over the sorted tasks and render each one
            sortedReviseTasks.map((task, index) => (
